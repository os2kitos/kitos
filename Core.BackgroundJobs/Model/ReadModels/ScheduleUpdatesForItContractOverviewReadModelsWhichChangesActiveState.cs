...
namespace Core.BackgroundJobs.Model.ReadModels
{
    ...
    ///     - Query the current state of the read models for IT-Contract and identify those who must be scheduled for an update since their Active state contains stale data
    ///
    /// Why do we need this?:
    ...
    /// </summary>
    public class ScheduleUpdatesForItContractOverviewReadModelsWhichChangesActiveState : IAsyncBackgroundJob
    {
        ...
        public string Id => StandardJobIds.ScheduleUpdatesForItContractOverviewReadModelsWhichChangesActiveState;

        public ScheduleUpdatesForItContractOverviewReadModelsWhichChangesActiveState(
        ...
        public Task<Result<string, OperationError>> ExecuteAsync(CancellationToken token = default)
        {
            ...
            var pendingReadModelUpdates = idsOfSourceEntitiesWhichHaveChangedState
                .Select(id => PendingReadModelUpdate.Create(id, PendingReadModelUpdateSourceCategory.ItContract))
                .ToList();
