...
namespace Tests.Integration.Presentation.Web.Tools
{
    public class OrganizationRegistrationHelper
    {
        ...
        public static async Task<HttpResponseMessage> SendGetRegistrationsAsync(int organizationId, int unitId, Cookie optionalLogin = null)
        {
            ...
            var orgUnitUrl = TestEnvironment.CreateUrl($"api/v1/organization-registrations/{organizationId}/{unitId}");
        ...
        {
            var cookie = optionalLogin ?? await HttpApi.GetCookieAsync(OrganizationRole.GlobalAdmin);
            var orgUnitUrl = TestEnvironment.CreateUrl($"api/v1/organization-registrations/unit/{organizationId}/{unitId}");
        ...
        {
            var cookie = optionalLogin ?? await HttpApi.GetCookieAsync(OrganizationRole.GlobalAdmin);
            var orgUnitUrl = TestEnvironment.CreateUrl($"api/v1/organization-registrations/{organizationId}/{unitId}/{targetUnitId}");
