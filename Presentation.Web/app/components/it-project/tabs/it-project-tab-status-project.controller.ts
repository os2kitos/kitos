(function (ng, app) {
    app.config(['$stateProvider', function ($stateProvider) {
        $stateProvider.state('it-project.edit.status-project', {
            url: '/status-project',
            templateUrl: 'app/components/it-project/tabs/it-project-tab-status-project.html',
                        ...
                        .then(function (rightResult) {
                            ...
                            return $http.get("api/itprojectrole/")
                                .then(function (roleResult) {
                                    ...
                                    //the resulting map
                                    var users = {};
                                    _.each(rights, function (right: { userId; user; roleName; }) {
    ...
    ['$scope', '$http', '$state', 'notify', '$modal', 'project', 'usersWithRoles', 'user',
        function ($scope, $http, $state, notify, $modal, project, usersWithRoles, user) {
            ...
            var prevPhase = null;
            _.each($scope.project.phases, function (phase: { id; updateUrl; prevPhase; }) {
