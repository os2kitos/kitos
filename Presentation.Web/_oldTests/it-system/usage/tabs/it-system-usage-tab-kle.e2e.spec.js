...
var PageObject = require("../../../../app/components/it-system/usage/tabs/it-system-usage-tab-kle.po");
describe("system usage tab kle", function () {
    var mockHelper;
    var browserHelper;
    var pageObject;
    ...
    describe("with write access", function () {
        ...
        it("should get tasks when main group is selected", function () {
            ...
            pageObject.mainGroupSelect.selectFirst();
            // assert
            expect(mock.requestsMade()).toMatchInRequests({ method: "GET", url: "api/itSystemUsage/1?(.)*taskGroup=[0-9]" });
        });
        ...
        it("should get selected tasks only on show selected click", function () {
            ...
            // assert
            expect(mock.requestsMade()).toMatchInRequests({ method: "GET", url: "api/itSystemUsage/1?(.)*tasks=true(.)*onlySelected=true" });
        });
        it("should check all tasks on confirmed select all pages click", function () {
            ...
            expect(mock.requestsMade()).toMatchInRequests({ method: "POST", url: "api/itSystemUsage/1?(.[^ ])*taskId=[^0-9]" });
        });
        it("should not check all tasks on dismissed select all pages click", function () {
            ...
            // assert
            // match api url with query string 'taskID=' only and ignoring any other query parameters
            expect(mock.requestsMade()).not.toMatchInRequests({ method: "POST", url: "api/itSystemUsage/1?(.[^ ])*taskId=[^0-9]" });
        });
        it("should check all tasks on page on confirmed select all click", function () {
            ...
            pageObject.taskRepeater.each(function (elem) {
                ...
                checkbox.getAttribute("id")
                    .then(function (id) {
                    return expect(mock.requestsMade()).toMatchInRequests({ method: "POST", url: "api/itSystemUsage/1?(.[^ ])*taskId=" + id });
                });
        ...
        it("should not check all tasks on page on dismissed select all click", function () {
            ...
            pageObject.taskRepeater.each(function (elem) {
                ...
                checkbox.getAttribute("id")
                    .then(function (id) {
                    return expect(mock.requestsMade()).not.toMatchInRequests({ method: "POST", url: "api/itSystemUsage/1?(.[^ ])*taskId=" + id });
                });
        ...
        it("should uncheck all tasks on page on confirmed deselect all click", function () {
            ...
            pageObject.taskRepeater.each(function (elem) {
                ...
                checkbox.getAttribute("id")
                    .then(function (id) {
                    return expect(mock.requestsMade()).toMatchInRequests({ method: "DELETE", url: "api/itSystemUsage/1?(.[^ ])*taskId=" + id });
                });
                expect(checkbox).not.toBeChecked();
            });
        });
        it("should not uncheck all tasks on page on dismissed deselect all click", function () {
            ...
            pageObject.taskRepeater.each(function (elem) {
                ...
                checkbox.getAttribute("id")
                    .then(function (id) {
                    return expect(mock.requestsMade()).not.toMatchInRequests({ method: "DELETE", url: "api/itSystemUsage/1?(.[^ ])*taskId=" + id });
                });
                expect(checkbox).toBeChecked();
            });
        });
        it("should uncheck all tasks on confirmed deselect all pages click", function () {
            ...
            // assert
            // match api url with query string 'taskID=' only and ignoring any other query parameters
            expect(mock.requestsMade()).toMatchInRequests({ method: "DELETE", url: "api/itSystemUsage/1?(.[^ ])*taskId=[^0-9]" });
        });
