...
namespace Presentation.Web.Controllers.API.V1
{
    ...
    public class HandoverTrialController : GenericApiController<HandoverTrial, HandoverTrialDTO>
    {
        ...
        protected override void RaiseUpdated(HandoverTrial item)
        {
            RaiseContractUpdated(item);
        }

        private void RaiseContractUpdated(HandoverTrial item)
        {
            var contract = _contractRepository.GetById(item.ItContractId);
            if (contract != null)
                DomainEvents.Raise(new EntityUpdatedEvent<ItContract>(contract));
        }

        protected override void RaiseDeleted(HandoverTrial entity)
        {
            RaiseContractUpdated(entity);
        }

        protected override void RaiseNewObjectCreated(HandoverTrial savedItem)
        {
