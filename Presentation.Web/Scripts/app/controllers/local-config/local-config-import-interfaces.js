(function (ng, app) {
    app.config([
        '$stateProvider', function ($stateProvider) {
            $stateProvider.state('local-config.import.interfaces', {
                url: '/interfaces',
                templateUrl: 'partials/local-config/import-template.html',
                controller: 'local-config.import.ImportInterfaceCtrl',
            });
        }
    ]);

    app.controller('local-config.import.ImportInterfaceCtrl', [
        '$rootScope', '$scope', '$http', 'notify', 'user',
        function ($rootScope, $scope, $http, notify, user) {
            $scope.url = 'api/excel?organizationId=' + user.currentOrganizationId + '&exportInterfaces';
            $scope.title = 'snitflader';
            ...
            $scope.submit = function () {
                ...
                $http.post('/api/excel?organizationId=' + user.currentOrganizationId + '&importInterfaces', formData, {
